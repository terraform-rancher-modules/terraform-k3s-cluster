...
resource "sshcommand_command" "install_k3s" {
  ...
  command     = "curl https://get.k3s.io | INSTALL_K3S_EXEC=\"server --node-external-ip ${var.node_public_ip} --node-ip ${var.node_internal_ip}\" INSTALL_K3S_VERSION=${var.kubernetes_version} sudo sh -"
